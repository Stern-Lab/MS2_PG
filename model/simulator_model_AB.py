...
def simulate(
    ...
    try:
        ...
        if simulate_sequence_sampling:
            ...
            wrangled_data  = wrangle_data_simplified(sequenced_passages)

        else:
            wrangled_data  = wrangle_data_simplified(passage)
        print("\n wrangled data \n", flush=True)
        ...
        if long_sumstat:
            ...
            sumstat = get_full_geno_sumstat_all_passages(wrangled_data)
            # [i for i in range(0, 11)]  # get_total_sumstat(data, [i for i in range(0, passages + 1)))
            print(f"\n got long for all passages: \n{len(wrangled_data)=}", flush=True)
        else:
            sumstat = get_expanded_sumstat_simplified(
                wrangled_data, [i for i in range(0, passages + 1)]
            )
            print(f"\n got short sumstat for all passages: \n{len(sumstat)=}", flush=True)
